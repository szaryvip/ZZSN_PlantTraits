...
class PGLSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        id = int(sample['id'])
        image = self.image_folder.loader(self.image_folder.root + "/0/" +
                                         str(id) + ".jpeg")
        ...
        try:
            targets =\
                sample[[f"{target}_mean" for target in self.targets]].values
        except KeyError:
            targets = None
        ...
        if targets is not None:
            features =\
                features.drop([f"{target}_mean" for target in self.targets] +
